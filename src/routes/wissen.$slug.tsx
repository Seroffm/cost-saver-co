...
const articles: Article[] = [
  {
    ...
    teaser: "Netzentgelte steigen, CO₂-Preis zieht an, aber Großhandelspreise sinken. Was bedeutet das für Ihre nächste Jahresabrechnung?",
  ...
  },
  {
    ...
    body: [
      ...
      { heading: "Festpreis oder flexibel?", paragraphs: [
  ...
  },
  {
    ...
    teaser: "Was Sie heute für eine 10-kWp-Anlage zahlen, wie schnell sie sich rechnet und welche Förderung greift.",
    image: imgSolar, tag: "Solar · Investition", read: "9 min", date: "1. Juni 2026", author: "Sarah Becker",
    body: [
      { heading: "Aktuelle Preise", paragraphs: [
      ...
      { heading: "Wann sich das rechnet", paragraphs: [
  ...
  },
  {
    ...
    teaser: "Wärmepumpe oder Nachtspeicher? Wir zeigen, ab wann ein separater Wärmestromzähler Sinn ergibt.",
    image: imgWaerme, tag: "Strom · Wärmepumpe", read: "6 min", date: "22. Mai 2026", author: "Mira Aydin",
    body: [
      { heading: "Separater Zähler oder Mischzähler?", paragraphs: [
  ...
  },
  {
    ...
    teaser: "Autostromtarif, dynamischer Tarif oder einfach Haushaltsstrom? Eine Entscheidungshilfe.",
    image: imgAuto, tag: "Strom · E-Mobilität", read: "7 min", date: "18. Mai 2026", author: "Sarah Becker",
    body: [
      { heading: "Welcher Tarif passt?", paragraphs: [
  ...
  notFoundComponent: () => (
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 py-24 text-center">
        ...
        <Button asChild className="mt-6"><Link to="/wissen">Zur Wissens-Übersicht</Link></Button>
      </div>
    </SiteLayout>
  ),
  ...
  return (
    <SiteLayout>
      <article className="mx-auto max-w-3xl px-4 py-12 md:py-20">
        <Link to="/wissen" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-success">
          <ArrowLeft className="h-4 w-4" /> Alle Artikel
        </Link>
        <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-success/10 px-3 py-1 text-xs font-semibold text-success">{a.tag}</div>
        <h1 className="mt-4 font-display text-4xl font-extrabold leading-tight text-primary md:text-5xl">{a.title}</h1>
        <p className="mt-4 text-lg text-muted-foreground">{a.teaser}</p>
        <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1.5"><User className="h-3.5 w-3.5" />{a.author}</span>
          <span className="inline-flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" />{a.date}</span>
          <span className="inline-flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" />{a.read} Lesezeit</span>
        </div>
          ...
          {a.body.map((section) => (
            <div key={section.heading} className="mt-8 first:mt-0">
              ...
              {section.paragraphs.map((p, i) => (
                <p key={i} className="mt-4 leading-relaxed text-foreground/90">{p}</p>
              ))}
            </div>
          ))}
        </div>
        ...
        <div className="mt-12">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {articles.filter((x) => x.slug !== a.slug).slice(0, 4).map((o) => (
              <Link key={o.slug} to="/wissen/$slug" params={{ slug: o.slug }} className="group rounded-xl border bg-card p-4 transition hover:border-success/40">
                <div className="text-xs font-semibold text-success">{o.tag}</div>
                <div className="mt-1 font-semibold text-primary group-hover:text-success">{o.title}</div>
              </Link>
            ))}
          </div>
        </div>
