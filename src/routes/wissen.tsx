...
const featured: Article = {
  ...
  teaser: "Netzentgelte steigen, CO₂-Preis zieht an, aber Großhandelspreise sinken. Was bedeutet das für Ihre nächste Jahresabrechnung? Unsere Analyse mit konkreten Spar-Empfehlungen.",
  image: imgStrom,
    ...
    <SiteLayout>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
          {categories.map((c, i) => (
            <motion.div key={c.label} {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.03 }}>
              <Link
                to={c.to}
              ...
              >
                <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${c.color} text-success ring-1 ring-success/20`}>
                  <c.icon className="h-7 w-7" />
                </div>
                <span className="text-sm font-semibold text-primary group-hover:text-success">{c.label}</span>
              </Link>
            </motion.div>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-8">
        <div className="mb-6 flex items-end justify-between">
          ...
          <Link to="/faq" className="text-sm font-semibold text-success hover:underline">Alle Themen <ArrowRight className="ml-1 inline h-4 w-4" /></Link>
        </div>
        <motion.article {...fadeUp} className="group grid overflow-hidden rounded-3xl border border-border bg-card shadow-card md:grid-cols-2">
          <div className="overflow-hidden">
            <img src={featured.image} alt="" loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
          </div>
          ...
          {articles.map((a, i) => (
            ...
            >
              <Link to="/wissen/$slug" params={{ slug: a.slug }} className="block">
                ...
                <div className="p-6">
                  <div className="text-[11px] font-bold uppercase tracking-wider text-success">{a.tag} · {a.read}</div>
                  <h3 className="mt-2 text-lg font-semibold leading-snug text-primary group-hover:text-success">{a.title}</h3>
