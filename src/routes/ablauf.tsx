...
import {
  FileSearch, PhoneCall, FileSignature, PlugZap, ArrowRight,
  ShieldCheck, Clock, CheckCircle2, Sparkles, Mail, BadgeCheck,
} from "lucide-react";
...
const steps = [
  ...
  },
  {
    ...
    bullets: ["Echte Menschen, kein Bot", "Unabhängig & ohne Provisionsdruck", "Auf Ihre Situation zugeschnitten"],
  },
  {
    icon: FileSignature,
    kicker: "Schritt 3",
...
const guarantees = [
  { icon: ShieldCheck, title: "Versorgungsgarantie", desc: "Der Wechsel ist gesetzlich abgesichert. Sie haben jederzeit Strom und Gas." },
  { icon: BadgeCheck, title: "Geprüfte Anbieter", desc: "Wir empfehlen ausschließlich Versorger mit nachweislich gutem Service." },
  { icon: Clock, title: "Wir halten Fristen ein", desc: "Kündigungsfristen und Wechseltermine werden punktgenau eingehalten." },
];
    ...
    <SiteLayout>
      ...
      <section className="relative overflow-hidden border-b border-border bg-gradient-to-b from-success/5 via-background to-background">
        ...
        <div className="mx-auto max-w-5xl px-4 py-20 md:py-28 text-center">
          <motion.div {...fadeUp} className="inline-flex items-center gap-2 rounded-full border border-success/30 bg-success/10 px-4 py-1.5 text-xs font-medium text-success">
            <Sparkles className="h-3.5 w-3.5" /> So einfach geht Tarifwechsel heute
          </motion.div>
          <motion.h1 {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.05 }} className="mt-6 text-4xl font-bold leading-tight text-primary md:text-6xl">
          ...
          <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className="mt-8 flex flex-wrap justify-center gap-3">
            <Button asChild size="lg" className="bg-success text-success-foreground hover:bg-success/90">
              <Link to="/">Jetzt Tarifprüfung starten <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
              ...
              <div className="relative flex items-start gap-5">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    <span className="rounded-full bg-success/10 px-2.5 py-0.5 font-semibold text-success">{s.kicker}</span>
                    <span className="inline-flex items-center gap-1 text-muted-foreground"><Clock className="h-3 w-3" /> {s.duration}</span>
                  </div>
      ...
      <section className="border-y border-border bg-muted/30">
        <div className="mx-auto max-w-6xl px-4 py-20 md:py-24">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold text-primary md:text-4xl">Was wir Ihnen garantieren</h2>
            <p className="mt-4 text-muted-foreground">Drei Versprechen, die für jeden Wechsel über EnergieClever gelten.</p>
          </div>
        ...
        <div className="rounded-3xl border border-border bg-gradient-to-br from-primary to-primary/90 p-8 text-primary-foreground shadow-card md:p-12">
          <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
            ...
            <div className="flex flex-col gap-3">
              <Button asChild size="lg" className="bg-success text-success-foreground hover:bg-success/90">
                <Link to="/">Tarifprüfung starten <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-white/30 bg-transparent text-primary-foreground hover:bg-white/10 hover:text-primary-foreground">
                <a href="mailto:hallo@energieclever.de"><Mail className="mr-2 h-4 w-4" /> Per E-Mail anfragen</a>
              </Button>
            </div>
          </div>
        </div>
