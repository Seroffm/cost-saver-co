...
import {
  ...
  ShieldCheck, Send, Building2, Sparkles,
} from "lucide-react";
  ...
  return (
    <SiteLayout>
      ...
      <section className="relative overflow-hidden border-b border-border bg-gradient-to-b from-success/5 via-background to-background">
        ...
        <div className="mx-auto max-w-4xl px-4 py-20 md:py-28 text-center">
          ...
          <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="mx-auto mt-5 max-w-2xl text-lg text-muted-foreground md:text-xl">
            Persönlich, kostenlos und ohne Verpflichtung. Wählen Sie den Weg, der für Sie am bequemsten ist.
          </motion.p>
        </div>
      </section>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {channels.map((c, i) => (
            ...
            >
              ...
              <div className="mt-5 text-xs uppercase tracking-wide text-muted-foreground">{c.label}</div>
              <div className="mt-1 text-xl font-bold text-primary group-hover:text-success">{c.value}</div>
              ...
              <div className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-success">
                Jetzt nutzen <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
              </div>
            </motion.a>
          ))}
        </div>
      ...
      <section className="border-y border-border bg-muted/30">
        <div className="mx-auto max-w-6xl px-4 py-20 md:py-24">
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            {/* Address */}
            <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.05 }} className="rounded-2xl border border-border bg-card p-7 shadow-soft">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-success/10 text-success">
            ...
            {/* Trust */}
            <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="rounded-2xl border border-border bg-gradient-to-br from-primary to-primary/90 p-7 text-primary-foreground shadow-card">
              ...
              <p className="mt-3 text-sm text-primary-foreground/80">
                Verarbeitung DSGVO-konform in Deutschland. Wir geben Ihre Daten niemals ohne Einwilligung weiter und rufen genau einmal zur Beratung an.
              </p>
              <Link to="/datenschutz" className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-success hover:underline">
      ...
      <section className="mx-auto max-w-5xl px-4 py-20 md:py-24">
        <motion.div {...fadeUp} className="rounded-3xl border border-success/30 bg-gradient-to-br from-success/10 via-card to-card p-8 shadow-card md:p-12">
