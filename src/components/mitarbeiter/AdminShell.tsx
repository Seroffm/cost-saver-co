...
import { LayoutDashboard, Users, Inbox, BarChart3, Settings, LogOut, Bell, Search, Zap, Building2, Tag, UserCog, Mail, CalendarClock, ChevronDown } from "lucide-react";
...
const navMain: NavEntry[] = [
  ...
  { to: "/mitarbeiter/statistiken", label: "Statistiken", icon: BarChart3, roles: ["admin", "manager"] },
];
...
const navAdmin: NavEntry[] = [
  ...
  { to: "/mitarbeiter/vorlagen", label: "E-Mail-Vorlagen", icon: Mail, roles: ["admin", "manager"] },
...
export function AdminShell({ children, title, subtitle, actions }: { children: ReactNode; title: string; subtitle?: string; actions?: ReactNode }) {
  ...
  const { user, users, switchUser, hasRole } = useMockAuth();
    ...
    <div className="min-h-screen bg-muted/30">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r bg-background lg:flex">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-3">
          {visibleMain.map((item) => <NavLink key={item.to} item={item} pathname={pathname} />)}
          {visibleAdmin.length > 0 && (
            <>
              <div className="px-3 pb-1 pt-4 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Verwaltung</div>
              {visibleAdmin.map((item) => <NavLink key={item.to} item={item} pathname={pathname} />)}
            </>
          )}
        </nav>
            ...
            <div className="relative">
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full z-40 mt-2 w-64 rounded-lg border bg-background p-2 shadow-lg">
                    <div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                      Rolle wechseln (Mockup)
                    </div>
                    {users.map((u) => (
                      <button
                        key={u.id}
                        ...
                        className={`flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-muted ${user.id === u.id ? "bg-muted" : ""}`}
                      >
                        <Avatar className="h-7 w-7"><AvatarFallback className="text-[10px]">{u.initials}</AvatarFallback></Avatar>
                        <div className="flex-1">
                          <div className="font-medium">{u.name}</div>
                          <div className="text-xs text-muted-foreground">{roleBadgeLabel[u.role]}</div>
                        </div>
                      </button>
                    ))}
                  </div>
  ...
  if (!hasRole(...allow)) {
    return fallback ?? (
      <div className="rounded-lg border border-dashed bg-muted/30 p-12 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Dieser Bereich ist nur für {allow.map((r) => roleBadgeLabel[r]).join(", ")} sichtbar.</p>
      </div>
    );
  }
  return <>{children}</>;
